...
data_suicide <- data_suicide %>%
  rename(Entity = GEO_NAME_SHORT, Year = DIM_TIME)
head(data_suicide)
```
...
```{r}
missing_values_mh <- colSums(is.na(dataMH_disorders))
print("Datos faltantes en datosMH:")
print(missing_values_mh)
```
...
```{r}
library(corrplot)
correlation_matrix <- cor(dataMH_disorders, use = "pairwise.complete.obs")
plot.new()
